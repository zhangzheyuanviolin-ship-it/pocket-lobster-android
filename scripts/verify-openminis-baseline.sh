...
grep -q 'versionCode = 341' "$ROOT_DIR/android/app/build.gradle.kts"
...
grep -q 'registerProvisionalThread' "$ROOT_DIR/src/composables/useDesktopState.ts"
grep -q 'CODEX_NOTIFICATION_HISTORY_LIMIT' "$ROOT_DIR/src/server/codexAppServerBridge.ts"

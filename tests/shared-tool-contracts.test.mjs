...
const manager = readFileSync('android/app/src/main/java/com/codex/mobile/CodexServerManager.kt', 'utf8')
const hostBridge = readFileSync('android/app/src/main/java/com/codex/mobile/SharedHostRuntimeBridge.kt', 'utf8')
...
const portPolicy = readFileSync('android/openminis/src/main/java/com/openminis/app/integration/PocketLobsterPortPolicy.kt', 'utf8')
...
assert.match(cli, /ANYCLAW_MINIS_BRIDGE_URL/)
assert.match(cli, /bridge-port/)
assert.match(cli, /ANYCLAW_SHARED_BRIDGE_TOKEN_FILE/)
assert.doesNotMatch(cli, /post\(18927,/)

assert.match(bridge, /browser\/schema/)
assert.match(bridge, /PocketLobsterPortPolicy\.minisBridgePort\(packageName\)/)
assert.match(portPolicy, /PRODUCTION_PACKAGE -> 0/)
assert.match(portPolicy, /OPERATOR_PACKAGE -> 10/)
assert.match(portPolicy, /BETA_PACKAGE -> 20/)
assert.match(bridge, /MinisRuntimeBridgeServer\(context\.applicationContext, port\(context\)\)/)
...
assert.match(claudeToolbox, /PERSISTED_MINIS_BRIDGE_PORT/)
assert.match(claudeToolbox, /SHARED_RUNTIME_DIR, \"bridge-port\"/)
...
assert.match(claudePrompt, /MinisRuntimeBridgeRuntime\.port\(this\)/)
assert.match(manager, /MinisRuntimeBridgeRuntime\.port\(context\)/)
assert.match(hostBridge, /shared-runtime\/bridge-port/)

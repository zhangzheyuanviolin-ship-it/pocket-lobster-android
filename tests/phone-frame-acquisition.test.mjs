...
assert.match(capture, /imagePresentationUs < expectedUs/)
assert.match(capture, /source.cropRect/)
assert.match(capture, /rowStride/)
assert.match(capture, /pixelStride/)
assert.match(capture, /latestBitmap\?\.recycle\(\)/)
const renderer = read(root + 'showerclient/src/main/java/com/ai/assistance/showerclient/ShowerVideoRenderer.kt')
assert.match(renderer, /getOutputImage\(index\)/)
assert.match(renderer, /COLOR_FormatYUV420Flexible/)
assert.match(renderer, /image.use/)

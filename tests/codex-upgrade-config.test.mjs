...
assert.match(gradle, /versionCode = 317/)
assert.match(gradle, /versionName = "1\.0\.76-codex-cli-0\.147\.0-gpt-5\.6-openminis-1\.12-accessible-collaboration-v317"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='317'/)
assert.match(workflow, /accessible-collaboration-v317-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 317/)

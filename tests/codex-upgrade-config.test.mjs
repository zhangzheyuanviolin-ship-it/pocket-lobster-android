...
assert.match(gradle, /versionCode = 338/)
assert.match(gradle, /versionName = "1\.0\.97-codex-cli-0\.147\.0-gpt-5\.6-openminis-1\.12-phone-ui-agent-manual-v338"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='338'/)
assert.match(workflow, /phone-ui-agent-manual-v338-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 338/)

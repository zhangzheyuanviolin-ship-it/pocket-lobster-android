...
assert.match(gradle, /versionCode = 355/)
assert.match(gradle, /versionName = "1\.0\.114-codex-cli-0\.153\.4-gpt-6-astra-ready-openminis-1\.12-provider-roundtrip-recovery-v355"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /provider-roundtrip-recovery-v355-beta/)
assert.match(workflow, /versionCode='355'/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 355/)

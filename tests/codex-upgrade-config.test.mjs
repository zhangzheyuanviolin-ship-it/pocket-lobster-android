...
assert.match(gradle, /versionCode = 333/)
assert.match(gradle, /versionName = "1\.0\.92-codex-cli-0\.147\.0-gpt-5\.6-openminis-1\.12-phone-ui-agent-runtime-v333"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='333'/)
assert.match(workflow, /phone-ui-agent-runtime-v333-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 333/)

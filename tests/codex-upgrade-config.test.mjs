...
assert.match(gradle, /versionCode = 346/)
assert.match(gradle, /versionName = "1\.0\.105-codex-cli-0\.153\.4-gpt-6-astra-ready-openminis-1\.12-phone-frame-acquisition-v346"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='346'/)
assert.match(workflow, /phone-frame-acquisition-v346-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 346/)

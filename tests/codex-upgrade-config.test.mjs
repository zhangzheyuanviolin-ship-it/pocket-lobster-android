...
assert.match(gradle, /versionCode = 342/)
assert.match(gradle, /versionName = "1\.0\.101-codex-cli-0\.153\.4-gpt-6-astra-ready-openminis-1\.12-history-frame-v342"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='342'/)
assert.match(workflow, /history-frame-v342-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 342/)

...
assert.match(gradle, /versionCode = 340/)
assert.match(gradle, /versionName = "1\.0\.99-codex-cli-0\.147\.0-gpt-5\.6-openminis-1\.12-phone-ui-agent-models-v340"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='340'/)
assert.match(workflow, /phone-ui-agent-models-v340-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 340/)

...
assert.match(gradle, /versionCode = 345/)
assert.match(gradle, /versionName = "1\.0\.104-codex-cli-0\.153\.4-gpt-6-astra-ready-openminis-1\.12-phone-context-capture-v345"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='345'/)
assert.match(workflow, /phone-context-capture-v345-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 345/)

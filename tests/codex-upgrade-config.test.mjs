...
assert.match(gradle, /versionCode = 349/)
assert.match(gradle, /versionName = "1\.0\.108-codex-cli-0\.153\.4-gpt-6-astra-ready-openminis-1\.12-shared-runtime-isolation-v349"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /shared-runtime-isolation-v349-beta/)
assert.match(workflow, /versionCode='349'/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 349/)

...
assert.match(gradle, /versionCode = 336/)
assert.match(gradle, /versionName = "1\.0\.95-codex-cli-0\.147\.0-gpt-5\.6-openminis-1\.12-phone-ui-agent-delegation-v336"/)
assert.match(gradle, /create\("operator"\)[\s\S]*applicationId = "com\.codex\.mobile\.pocketlobster\.test"/)
assert.match(workflow, /versionCode='336'/)
assert.match(workflow, /phone-ui-agent-delegation-v336-beta/)
assert.match(workflow, /PACKAGE_ID="com\.codex\.mobile\.pocketlobster\.beta"/)
assert.match(baselineScript, /versionCode = 336/)

...
test('collaboration coordinator routes through the leader before selectively invoking real agents', async () => {
  ...
  assert.match(server, /detectRequiredCollaborationTargets/)
  assert.match(server, /decisionMissingRequiredTargets/)
  assert.match(server, /enforceRequiredCollaborationDecision/)
  assert.match(server, /宿主已按用户明确要求补齐协作成员/)
  assert.match(server, /宿主已按用户补充要求补齐协作成员/)
  assert.match(server, /成员消息由宿主负责投递/)
  assert.match(server, /禁止扩展为环境自检、文件写入或共享工作区测试/)

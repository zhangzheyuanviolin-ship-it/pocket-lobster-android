    ...
    fun system(protocol: PhoneUiModelProtocol): String {
        if (protocol == PhoneUiModelProtocol.GUI_PLUS_NATIVE) {
            return """
                ...
                Always inspect the new screenshot after an action. Treat only the latest screenshot as evidence of the current page. Carry out ordinary user-requested actions directly. Do not invent dialogs, controls, page text, or extra requirements from the app name, prior reasoning, or an earlier screenshot. Request help only when a control visibly requires information that the user has not provided.
        ...
        return """
            ...
            执行前确认当前页面；Type完成后宿主会尝试自动收起输入法，必须观察下一张截图重新定位发送或确认按钮，绝不能复用输入文字前的旧坐标；如果目标文字已经显示在输入框中，不得再次Type或清空重输，应直接寻找发送按钮。必须区分纵向页面滚动和横向栏目切换：查看页面更下方内容时，手指应从屏幕下方中央向上方中央滑动；查看更上方内容时反向滑动；只有明确看到横向列表、轮播或分页控件时才左右滑动。每次Swipe后必须先检查新截图中刚出现的文字、项目和位置变化，再决定下一步；只有目标仍未出现且截图证明页面确实移动时，才继续同方向滑动。Swipe没有带来页面变化时，只允许换起点并增大距离重试一次；仍无效时应尝试反方向、其他可滚动区域、搜索或筛选，或者如实finish说明未找到，绝不能机械重复到耗尽剩余步数。点击一次没有页面变化时先等待一次，再根据新截图换控件或换策略，不得反复点击同一个坐标。同一静止页面最多连续Wait三次。Home只有用户任务明确要求进入桌面时才允许使用。Launch、Tap、Type、Swipe、Wait、Back、Double Tap和Long Press都不得在没有新证据时机械循环。执行下一步前必须检查上一步是否生效；进入无关页面先Back，Back无效再点击可见返回或关闭按钮；完成前核对结果。finish的message必须用二到五句简洁说明实际执行了什么、在屏幕上观察到什么结果、是否仍有未完成事项，不得只写“完成”或一句泛化成功提示。当前页面以这次新截图为准，历史判断不代表当前事实。不得根据应用名称或历史推测编造弹窗、页面文字或额外步骤。用户要求的普通操作应直接执行。只有最新截图中的可见控件需要用户尚未提供的信息时，才允许Take_over。看到口袋大龙虾虚拟屏幕工作区时，应启动用户指定的应用。
        """.trimIndent()
    }
}
...
object PhoneUiAgentModelClient {
    ...
    fun decide(
        ...
        val prompt = buildString {
            ...
            if (screenInfo.isNotBlank()) append("\n$screenInfo")
